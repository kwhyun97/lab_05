...
str_match(x, pattern = "^My.*successful$")
```
...
str_match(x, pattern = "(^[:alpha:]{3}).\\d{2,4}")
```
...
```{r}
str_match(x, pattern = "([:alpha:]{3}).(\\d{4})?.*")
```
...
str_match(x, pattern = "[[:punct:]]?\\d*[[:punct:]]?\\d*[[:alpha:]]?\\d$")[, 1]
```
